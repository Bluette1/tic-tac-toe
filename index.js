...
const Game = () => {
  ...
  const checkValid = (posn) => board.checkValid(posn);
...
const displayBoard = (game) => {
  board.textContent = '';
  const stringBoard = game.printBoard().split('\n');
...
const play = (game, reset) => {
  ...
  choosePosnBtn.addEventListener('click', (evt) => {
    ...
    const position = chosenPosn.value;
    const verified = game.verifyUpdate(position);
